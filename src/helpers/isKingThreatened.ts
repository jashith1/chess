...
export default function isKingThreatened(turn: string, pieceToIgnore?: string, kingLocOverride?: string, setCheck?: Dispatch<SetStateAction<string[][]>>) {
	...
	positionStack.forEach((piece) => {
		document.querySelectorAll(`.${enemy}${piece}`).forEach((pieceElement) => {
			...
			const isCheck = movementCalculation(enemy, piece, row, col, pieceToIgnore, [], checkTemp, kingLoc);
